...
#
# The Common Name(CN) will be created using the alias and the CN of cloud
# certificate
#
# @param $1 Path to root certificate keystore.
# @param $2 Common Name of root certificate.
...
# @param $6 Desired Alias of system certificate.
create_system_keystore() {
  ...
  if [ ! -f "${SYSTEM_KEYSTORE}" ]; then
    ...
    keytool -genkeypair -v \
      ...
      -keypass:env "PASSWORD" \
      -dname "CN=${SYSTEM_KEY_ALIAS}.${CLOUD_KEY_ALIAS}" \
